"""
Nested leave-one-site-out training for the SUBJECT-ADAPTIVE HeteroGNN.

This is train_nested_loso.py with one thing changed: the fixed group-level
covariation adjacency is replaced by a per-subject top-k graph learned from node
embeddings (pilot_adaptive_graph.AdaptiveHeteroGNN). Everything else -- the
ComBat/scaler discipline, the outer/inner split structure, temperature scaling,
ECE, the resume logic -- is identical, so the two runners' results.csv files are
directly comparable and only the graph construction differs.

    for each held-out site S:                    # OUTER: 20 folds
        inner GroupKFold over the remaining 19   # HP chosen here, S never seen
            -> best config
        refit on all training sites, early-stop
        and temperature-scale on a TRAIN-only split
        -> evaluate once on S

The adaptive model ignores the edges build_fold() computes and rebuilds its own
per-subject graph in forward(), so build_fold is used purely to produce the same
harmonized, scaled node features the fixed model sees. tau is therefore fixed and
irrelevant here; k (neighbours per node) is the adaptive graph's real knob and is
searched instead.

Hyperparameter search (the user's ask: depth + adaptive knob):
    layers  in {2, 3, 4}      # deeper message passing now on the grid
    k       in {5, 10}        # top-k neighbours per node in the learned graph
    hidden  = 64              # fixed at the pilot's working width
    dropout = 0.3

Run:
    python train_nested_loso_adaptive.py --smoke   # 2 sites, 1 config, 1 seed
    python train_nested_loso_adaptive.py           # full run (hours; resumable)
"""
...
from hetero_gnn import fit_temperature, expected_calibration_error
...
# hidden fixed at the pilot's working width; depth and neighbourhood searched.
HP_GRID = {"hidden": [64], "layers": [2, 3, 4], "k": [5, 10], "dropout": [0.3]}
TAU_FIXED = 0.3   # only affects the unused fixed edges; node features are tau-free
...
def main():
    ...
    if args.smoke:
        grid = grid[:1]
    print(f"HP grid: {len(grid)} configs  seeds: {len(seeds)}")

    res_path = os.path.join(OUT_DIR, "nested_loso_adaptive_results.csv")
    hp_cache_path = os.path.join(OUT_DIR, "nested_loso_adaptive_hp.json")
    done = set()
    ...
    hp_cache = {}
    if os.path.exists(hp_cache_path) and not args.smoke:
        hp_cache = json.load(open(hp_cache_path))
        print(f"resuming: {len(hp_cache)} cached inner-HP choices")
    ...
    for site in outer_sites:
        ...
        # ── INNER: HP selection over TRAINING sites only (cached for resume) ──
        if site in hp_cache:
            best_hp = hp_cache[site]["hp"]
            best_score = hp_cache[site]["inner_auc"]
            print(f"\n[{site}] cached inner-best hp={best_hp} inner_auc={best_score:.3f}")
        else:
            ...
            if len(grid) > 1:
                ...
                for hp in grid:
                    ...
                    print(f"  [{site}] hp={hp} inner_auc={s:.3f}")
                    if s > best_score:
                        best_score, best_hp = s, hp
            print(f"[{site}] inner-best hp={best_hp} inner_auc={best_score:.3f}")
            if not args.smoke:
                hp_cache[site] = {"hp": best_hp, "inner_auc": best_score}
                json.dump(hp_cache, open(hp_cache_path, "w"), indent=2)

        # ── OUTER: refit on all training sites, evaluate once on held-out site ──
